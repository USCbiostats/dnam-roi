...
samples <- c("DA", "DB", "IA", "IB", "HA", "HB", "JA", "JB", "MA", "MB","JN","IN")

# The blacklist file (gzipped BED)
blacklist_file <- file.path(data_dir, "hg38-blacklist.v2.bed.gz")

#############################################################
# 2) Read the blacklist as a GRanges
#############################################################
blacklist_gr <- import(blacklist_file, format = "BED")
seqlevelsStyle(blacklist_gr) <- "UCSC"  # Ensure consistent naming, if needed

#############################################################
# 3) Define a function to read and filter a WGBS bed file
#############################################################
read_and_filter_wgbs <- function(bed_file, blacklist_gr) {
  # bed_file: path to the .bed file
  # blacklist_gr: GRanges for the blacklist
  ...
  filtered_gr <- subsetByOverlaps(meth_gr, blacklist_gr, invert = TRUE)
...
for (smp in samples) {
  ...
  # 5a) Read & Filter
  filtered_gr <- read_and_filter_wgbs(bed_file, blacklist_gr)
...
# write.csv(final_summary, file = "WGBS_summary_postBlacklist.csv", row.names = FALSE)
